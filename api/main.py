...
# This path is where the API will look for your model in its temporary file system
...
async def lifespan(app: FastAPI):
    ...
    # Note: Ensure your .keras file is in your GitHub repo or use a download function here
    if os.path.exists(WEIGHTS_PATH):
        clinical_model = build_asteri_brain()
        clinical_model.load_weights(WEIGHTS_PATH)
        print("Diagnostic Core Online.")
    else:
        print(f"CRITICAL: Model file not found at {WEIGHTS_PATH}")
    yield
    clinical_model = None
